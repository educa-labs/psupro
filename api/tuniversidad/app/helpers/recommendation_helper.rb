module RecommendationHelper
  ...
  def random_select(user,n,essay,area)
    if area == 'compute'
      recs = Recommendation.where(user_id:user.id, seen:false, essay:essay, computed_area:true).includes(carreer:[:campu,:weighing,:area,{university: :institution}])
    else
      recs = Recommendation.where(user_id:user.id, seen:false, essay:essay, area_id:area.to_i).includes(carreer:[:campu,:weighing,:area,{university: :institution}])
    end
